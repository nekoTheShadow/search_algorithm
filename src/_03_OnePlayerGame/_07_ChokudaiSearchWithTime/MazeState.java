...
public class MazeState implements Comparable<MazeState> {
    ...
    public static int chokudaiSearchActionWithTimeThreshold(MazeState state, int beamWidth, int beamDepth, long timeThreshold) {
        TimeKeeper timeKeeper = new TimeKeeper(timeThreshold);
        
        List<PriorityQueue<MazeState>> beam = new ArrayList<>();
        for (int i = 0; i < beamDepth+1; i++) {
            beam.add(new PriorityQueue<>());
        }
        beam.get(0).add(state);
        
        for (int cnt = 0; ; cnt++) {
            for (int t = 0; t < beamDepth; t++) {
                PriorityQueue<MazeState> nowBeam = beam.get(t);
                PriorityQueue<MazeState> nextBeam = beam.get(t+1);
                for (int i = 0; i < beamWidth; i++) {
                    if (nowBeam.isEmpty()) {
                        break;
                    }
                    
                    MazeState nowState = nowBeam.peek();
                    if (nowState.isDone()) {
                        break;
                    }
        ...
        for (int t = beamDepth; t >= 0; t--) {
            PriorityQueue<MazeState> nowBeam = beam.get(t);
            if (!nowBeam.isEmpty()) {
                return nowBeam.peek().firstAction;
            }
        }
        return -1;
    }

    public static int chokudaiSearchAction(MazeState state, int beamWidth, int beamDepth, int beamNumber) {
        List<PriorityQueue<MazeState>> beam = new ArrayList<>();
        for (int i = 0; i < beamDepth+1; i++) {
            beam.add(new PriorityQueue<>());
        }
    ...
    public static int END_TURN = 100;
